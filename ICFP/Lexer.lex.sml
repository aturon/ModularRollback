structure InternalLexer  = struct
...
let
fun yyAction0 (strm, lastMatch : yymatch) = (yystrm := strm;  VAL)
fun yyAction1 (strm, lastMatch : yymatch) = (yystrm := strm;  FUN)
fun yyAction2 (strm, lastMatch : yymatch) = (yystrm := strm;  LPAREN)
fun yyAction3 (strm, lastMatch : yymatch) = (yystrm := strm;  RPAREN)
fun yyAction4 (strm, lastMatch : yymatch) = (yystrm := strm;  EQ)
fun yyAction5 (strm, lastMatch : yymatch) = (yystrm := strm;  PLUS)
fun yyAction6 (strm, lastMatch : yymatch) = (yystrm := strm;  SEMI)
fun yyAction7 (strm, lastMatch : yymatch) = (yystrm := strm;  NUM)
fun yyAction8 (strm, lastMatch : yymatch) = (yystrm := strm;  ID)
        ...
        | SOME(inp, strm') =>
            if inp = 0wx5B
              ...
              then if inp = 0wx3A
                  ...
                  then if inp <= 0wx2F
                      then yyAction8(strm, yyNO_MATCH)
                      else yyQ8(strm', yyMATCH(strm, yyAction8, yyNO_MATCH))
                else if inp <= 0wx40
                  then yyAction8(strm, yyNO_MATCH)
                  else yyQ8(strm', yyMATCH(strm, yyAction8, yyNO_MATCH))
            else if inp = 0wx6C
